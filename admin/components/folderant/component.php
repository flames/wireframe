...
function get_installed_modules($path,$installedModules = FALSE,$submodules = FALSE){
    ...
    global $DIR_ROOT;
    global $permissions;
    $comp_path = "components/folderant";
    if ($installedModules){ 
            ...
            while (false !== ($file = readdir($handle))) {
                ...
                if (is_dir($comp_path."/modules/".$path."/".$file) && isset($installedModules[$file]) && $installedModules[$file]) {
                    ...
                    if ($permissions->hasPermission($modules_temp["permission"])){
                        ...
                        $modules[$file]["url"] = $URL_ROOT."admin/folderant".$path."/".$file."/";
        ...
        foreach($submodules as $sub){
            if (is_dir($comp_path."/modules".$path."/".$sub)) {
                ...
                if ($permissions->hasPermission($modules_temp["permission"])){
                    ...
                    $modules[$sub]["url"] = $URL_ROOT."admin/folderant".$path."/".$sub."/";
...
function make_list($modules,$active_module, $parent = FALSE){
    ...
    $installedModules = $DB->select_pair("modules","name","active","order",false, "component = 8");
    ...
    $breadcrump = '<li><a href="'.$URL_ROOT.'admin/folderant/">folderant</a></li>';
    $modul_temp = $modules;
    $first = TRUE;
    ...
    if ($modul_temp["file"] != "" && $modul_temp["path"] != ""){
        $active_modul = "components/folderant/modules".$modul_temp["path"].$modul_temp["file"];
        if($_GET["edit"]) $active_modul .= "_edit";
        $active_modul .= ".php";
    }
    $stats = "components/folderant/stats/stats.php";
?>