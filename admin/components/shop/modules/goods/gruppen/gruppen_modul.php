...
class gruppen_modul extends modul{   
    ...
    public function edit_buttons($id,$css_id){
        ...
        if ($this->permissions->hasPermission($this->permission.".edit")) $html.='
                ...
                <button class="btn btn-info start" onclick="open_iframe(\''.$this->base_url.'admin/components/shop/modules/goods/gruppen/attributes.php?id='.$id.'\',600,600);">
    ...
    public function getEditField_Options($id,$field,$entity){
        ...
        $item_options = $this->db->select("SELECT * FROM `".$field[2]."` WHERE `group` IS NULL AND `entity_type` =0 AND `entity_id` =".$entity["id"]." ORDER BY id;",MYSQLI_ASSOC,FALSE,"id");
        $html .= '
        ...
        foreach($item_options as $option_id => $item_option){
            ...
            $html .='
                     <a style="float:right;margin-right:5px;" class="btn btn-success" data-toggle="modal" href="#new_sub_option_'.$option_id.'" ><i class="icon-plus icon-white"></i>Neue Option</a>';
            $html .='
            ...
            if($item_option["type"] == 0){
                        ...
                        $childs = $this->db->select("SELECT * FROM ".$field[2]." WHERE `type` IS NULL AND `group` =".$option_id.";");
                        $html .= '
                            Name: <input type="text" name="save['.$field[1].']['.$option_id.'][name]" value="'.$item_option["name"].'" />&nbsp;&nbsp;&nbsp;
                            Type: 
...
\'\\                            </div>\';
    ...
    public function save_field($id, $field, $table, $first){
                if ($field[0] == "Uploads"){}
                    ...
                    $this->db->query("DELETE FROM ".$field[2]." WHERE `entity_type` = 0 AND `entity_id` = $id ;");
                    if($_POST["save"][$field[1]])
                    foreach($_POST["save"][$field[1]] as $item_option){
                        if($item_option["save"] == 1){
                            if($item_option["type"] == 0)
                                $this->db->query("INSERT INTO `".$field[2]."` (`id`, `type`, `group`, `entity_type`, `entity_id`, `name`, `operation`, `value`) VALUES (NULL, '".$item_option["type"]."', NULL, '0', '".$id."', '".$item_option["name"]."', '".$item_option["operation"]."', '".$item_option["value"]."');");
                            else{
                                $this->db->query("INSERT INTO `".$field[2]."` (`id`, `type`, `group`, `entity_type`, `entity_id`, `name`, `operation`, `value`) VALUES (NULL, '".$item_option["type"]."', NULL, '0', '".$id."', '".$item_option["name"]."', NULL, NULL);");
                                $group_id = $this->db->lastindex();
                                if($item_option["childs"])
                                foreach($item_option["childs"] as $child_id => $child){
                                    if($child["save"] == 1)
                                    $this->db->query("INSERT INTO `".$field[2]."` (`id`, `type`, `group`, `entity_type`, `entity_id`, `name`, `operation`, `value`) VALUES (NULL, NULL, ".$group_id.", '0', '".$id."', '".$child["name"]."', '".$child["operation"]."', '".$child["value"]."');");
                                }
                            }  
                        }
                    }
