...
if (isset($_POST["save"])){
	$DB->query("DELETE FROM wf_group_attr WHERE group_id = ".$_GET["id"]." AND type = 0;");
	$query = "INSERT INTO `wf_group_attr` (`id`, `group_id`, `name`, `type`) VALUES ";
	$first = true;
	foreach($_POST["save_attr"] as $attr){
	if(!$first) $query .= ", ";
	$query .= "(NULL, '".$_GET["id"]."', '".$attr."', 0)";
	$first = false;
	}
	$query .= ";";
	echo $query;
...
$attributes = $DB->select("SELECT * FROM wf_group_attr WHERE group_id = ".$_GET["id"]." AND type = 0 ORDER by id;");
?>
