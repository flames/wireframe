...
class FTP
{
	...
	*/
	 ...
	 * @param  string  $dest Path to Source Folder
	 * @param  bool    $recursive recursive or not
	 * @param  string  $without auszulassender Ordner (TODO zum Array umbauen)
	 * @return bool
	 */
        public function copyFolder($source, $dest, $recursive = TRUE ,$without = FALSE)
        {
            $work_dir = str_replace($_SESSION["_registry"]["root"], '', $dest);
            if ($work_dir != ""){
                self::makeDir($work_dir); 
                self::chmod($work_dir, 0755);
                $response .= $dest." erstellt<br>";
            }
            ...
            while ($file = @readdir ($handle))
            {
                ...
                {
                    if ($file != $without){
                        $response .= self::copyFolder($source.$file."/", $dest.$file."/", $recursive,$without);
                    }
                    else continue;
                }
                else
                {
                    $destFile = str_replace($_SESSION["_registry"]["root"], '', $dest.$file);
                    self::upload( $source.$file,  $destFile, 'auto', 0 );
                    self::chmod($destFile, 0755);
                    $response .= $source.$file." copied to ".$dest.$file."<br>";
                }
             }
            @closedir($handle);
            return $response;
