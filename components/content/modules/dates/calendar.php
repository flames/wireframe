...
exit();
	...
	$prev = prev($content);
	if(!is_numeric($prev)){
		$dates = $DB->select("SELECT * FROM `wf_calendar_dates` WHERE feed_id = ".$page["view_options"]." AND `to` >= NOW() AND status = 1 ");
		foreach ($dates as $date){
			$output .= '<div style="cursor:pointer;" onclick="location=\''.$date["id"].'/'.urlencode(str_replace('/', '//', $date["headline"])).'/\'"><h1>'.$date["headline"].' ('.$time->convertDate($date["from"]).' - '.$time->convertDate($date["to"]).')</h1>
			'.$date["short_text"].'</div>';
		}
	}
	else{
