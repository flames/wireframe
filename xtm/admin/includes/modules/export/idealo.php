...
//Version of module
$version_number_idealo = '2.4.3';
$version_date = '14.05.2012';
	...
	$new_version_idealo = explode ( '.', $version_idealo );
	   ...
	   $idealo_version_text_no_modified = '<blink>' .TEXT_NEW_IDEALO_MODULE_01 . $version_idealo .TEXT_NEW_IDEALO_MODULE_02 . ' <a href="' . $idealo_module_download . '" target="_newtab"><b>zur Download-Seite</b></a></blink>';	
		...
		if ( count ( $old_version_idealo ) == count ( $new_version_idealo ) ){
		   	for ( $i_idealo = 0; $i_idealo < count ( $old_version_idealo ); $i_idealo++ ){
				if ( ( int ) $old_version_idealo [ $i_idealo ] < ( int ) $new_version_idealo [ $i_idealo ] ){
					if ( $i_idealo > 0 ){
						if ( ( int ) $old_version_idealo [ $i_idealo - 1 ] <= ( int ) $new_version_idealo [ $i_idealo - 1 ] ) {
...
define('DILEVERYCOSTS', 'F&uuml;gen Sie bitte Ihre Pauschalen Versandkosten ein');
...
//include 'backup/zip.php';
  class idealo{
    ...
    public function __construct() {
      ...
      $this->description = '<center><a href="http://www.idealo.de" target="_blank"><img src = "http://www.idealo.de/pics/logos/logo_idealo_de.png"></a></center>';
...
//      $this->checkLoworderfee();
    ...
    public function process($file) {
	        ...
	        $export_query =xtc_db_query("SELECT
				...
				$cat = $this->getCategory(xtc_get_product_path($products['products_id']));
	        	...
	        	if($this->filter($products['products_id'], $products['manufacturers_name']) === true && $this->filterCat($cat) === true){
					...
					$products_description = preg_replace($Regex, $Ersetzen, $products_description);
					...
					$products_short_description = preg_replace($Regex, $Ersetzen, $products_short_description);
					$products_short_description = html_entity_decode($products_short_description);
		            ...
		            $schema .= QUOTECHAR . $products['products_name'] . QUOTECHAR . SEPARATOR .
...
//		                       QUOTECHAR . substr($cat,0,strlen($cat)-2) . QUOTECHAR. SEPARATOR .
			       ...
			       foreach($this->payment as $payment) {
		       			if($payment['active'] === true){
		       				$schema .= QUOTECHAR . round($this->getPaymentCosts($payment['db'], $price, $shipping),2) . QUOTECHAR . SEPARATOR;	
		       			}
		       		}
	...
	public function getDelivery($price, $max, $weight){
		$delivery = explode(";",$this->shippingcosts);
		if(count($delivery) > 1){
