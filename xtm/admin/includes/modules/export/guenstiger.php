...
define('MODULE_GUENSTIGER_FILE_DESC' , 'Das Modul legt diese generierte Preisliste/Produktdatenliste/Exportdatei bei der Speicherart \'Auf dem Server speichern\' automatisch im Unterverzeichnis Ihres Shops mit dem Namen \'export/\' (Das Verzeichnis ben&ouml;tigt folgende Rechte 0777) ab. Die Datei hat dort folgende Bezeichnung:');
define('MODULE_GUENSTIGER_STATUS_DESC','Modulstatus');
define('MODULE_GUENSTIGER_STATUS_TITLE','Status');
define('MODULE_GUENSTIGER_EXPORT_YES','Auf dem Server speichern und herunterladen');
define('MODULE_GUENSTIGER_EXPORT_NO','Auf dem Server speichern');
define('MODULE_GUENSTIGER_EXPORT_LINK','export/');
...
$res = xtc_db_fetch_array($gtgoogle_input_query);


if (isset($_POST['gtgoogle'])) {
  $_POST['gtgoogle'] == "yes"?$val="Y":$val="N";
  if( $res !== false ) {
    ...
    if( $val != $res['configuration_value'] ) {
      xtc_db_query("update " . TABLE_CONFIGURATION . "
                ...
                where configuration_key = 'MODULE_GUENSTIGER_PSMGOOGLEV'");
    }
  } else {
    // insert data
    xtc_db_query("insert into " . TABLE_CONFIGURATION . "
            ...
            values ('MODULE_GUENSTIGER_PSMGOOGLEV', '" . $val . "', 6, 1, '', now()) ");
  }
  $UseGoogle = $val=="Y";
} else {
  if( $res !== false ) {
  ...
  class guenstiger {
    ...
    function guenstiger() {
      ...
      $this->code = 'guenstiger';
      $this->title = MODULE_GUENSTIGER_TEXT_TITLE;
      $this->description = MODULE_GUENSTIGER_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_GUENSTIGER_SORT_ORDER;
      $this->CAT=array();
      $this->PARENT=array();
      $this->enabled = ((MODULE_GUENSTIGER_STATUS == 'True') ? true : false);
  ...
  function checkStandardShippingCostsOption() {
    ...
    if($this->checkShippingCostOption('TABLE') > 0) {
      ...
      $this->paymentTable = true;
      $this->setPaymentParcelServicesValues('DP');
    } elseif($this->checkShippingCostOption('UPS') > 0) {
      $this->paymentTable = true;
      $this->setPaymentParcelServicesValues('UPS');
    } elseif($this->checkShippingCostOption('UPSE') > 0) {
      $this->paymentTable = true;
      $this->setPaymentParcelServicesValues('UPSE');
    }
  }

  function setPaymentParcelServicesValues($name) {
    $explodedValues = array();
    ...
    // take the data from the db
    $getValues = xtc_db_query("SELECT RIGHT(`configuration_key`,1)  AS `table_values` ".
                              "FROM ". TABLE_CONFIGURATION ."  ".
                              "WHERE `configuration_key` LIKE '%SHIPPING_".$name."%' ".
                              "AND `configuration_value` LIKE '%DE%'");
    ...
    $getValues = xtc_db_query("SELECT `configuration_value` AS `table_values`
                         FROM " . TABLE_CONFIGURATION . "
                         WHERE `configuration_key` LIKE 'MODULE_SHIPPING_".$name."_COST_".$result['table_values']."';");
   ...
   function checkShippingCostOption($option) {
    ...
    function process($file) {
        ...
        $UseGoogle ? $gtgoogle_link="utm_source=preissuchmaschine&utm_medium=cpc&utm_campaign=preissuchmaschine&" : $gtgoogle_link="";
      ...
      // get product categorie
          ...
          $image_if_available = '';
            ...
            $schema .= $products['products_id'] .'|'.
                       ...
                       HTTP_CATALOG_SERVER . DIR_WS_CATALOG . 'product_info.php?'.$_POST['campaign']. $gtgoogle_link .xtc_product_link($products['products_id'], $products['products_name']). '|' .
                       $image_if_available . '|' .
  ...
  if (!$PSMFound) {
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = xtc_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_GUENSTIGER_STATUS'");
    ...
    function install() {
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_GUENSTIGER_FILE', 'preissuchmaschine.csv',  '6', '1', '', now())");
      xtc_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_key, configuration_value,  configuration_group_id, sort_order, set_function, date_added) values ('MODULE_GUENSTIGER_STATUS', 'True',  '6', '1', 'xtc_cfg_select_option(array(\'True\', \'False\'), ', now())");
}
    ...
    function keys() {
      return array('MODULE_GUENSTIGER_STATUS', 'MODULE_GUENSTIGER_FILE');
    }
