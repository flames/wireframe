<?php
  /* -----------------------------------------------------------------------------------------
   $Id: create_account.php 2064 2011-07-28 13:24:50Z dokuman $
  ...
  if ($_GET['action'] == 'edit') {
    ...
    if (xtc_get_geo_zone_code($entry_country_id) != '6') {
      ...
      if ($vatID->vat_info['vat_id_status'] == '0'){
        $entry_vat_error_text = TEXT_VAT_FALSE;
      }

      if ($vatID->vat_info['vat_id_status'] == '1'){
        $entry_vat_error_text = TEXT_VAT_TRUE;
      }

      if ($vatID->vat_info['vat_id_status'] == '8'){
        $entry_vat_error_text = TEXT_VAT_UNKNOWN_COUNTRY;
      }


      if ($vatID->vat_info['vat_id_status'] == '9'){
        $entry_vat_error_text = TEXT_VAT_UNKNOWN_ALGORITHM;
      }
                          ...
                          <?php
                            if ($error == true) {
                              if ($entry_company_error == true) {
                                echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="64"').'&nbsp;'.ENTRY_COMPANY_ERROR;
                              } else {
                                echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="64"');
                              }
                            } else {
                              echo xtc_draw_input_field('entry_company', $entry_company, 'maxlength="64"');
                            }
