...
jQuery.fn.autocomplete = function(url, settings ) 
{
	return this.each( function()//do it for each matched element
	{
		...
		var list = valueInput.next().css({top: textInput.offset().top + textInput.outerHeight(), left: textInput.offset().left, width: 'auto'});
		var oldText = '';
		var typingTimeout;
		var size = 0;
		var selected = -1; //original 0
		...
		function getData(text)
		{
			...
			if (text != oldText && (settings.minChars != null && text.length >= settings.minChars))
			{
				...
				if($('#fulltext').attr('checked')) settings.parameters.fulltext = 1;
				
				if($('#use_short_tags').attr('checked')){
					var tags = new Array();
					$.each($("input[@name='short_tags[]']:checked"), function() {
        				tags.push($(this).val()) ;
					});
					var json_tags = tags.join();
					settings.parameters.tags = json_tags;
				}
				$.getJSON(url,settings.parameters,function(data)
				{
					...
					if (data)
					{
						...
						for (i = 0; i < data.length; i++)//iterate over all options
						{
						  ...
						  {	
								items += '<li content="' + key + '">' + data[i][key].replace(new RegExp("(" + text + ")","i"),"<strong>$1</strong>") + '</li>';
						  }
						  list.html(items);
						  ...
						  click(function () {
						  	valueInput.val( $(this).attr('content') );
						  	textInput.val($(this).attr('content') );
						  	custom($(this).attr('content'));
						  	clear();
						  });
						}
		...
		textInput.keydown(function(e) 
		{
			...
			{ 
				...
				if(valueInput.val()=="") {
					...
					valueInput.val( list.children().eq(selected).attr('content') );
				}
				custom(valueInput.val()); ///////////
				return false;
			}
